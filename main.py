...
def run() -> None:
    ...
    for panel in PANELS:
        ...
        # File button
        window[panel]["file button"] = gui.Button(
            window[panel]["frame"], width=WIDTH["button"], height=HEIGHT["path"]
        )
        window[panel]["file button"].grid(column=0, row=1, padx=MARGIN, pady=MARGIN)
        if panel == "source":
        ...
        # File label
        window[panel]["file label"] = gui.Label(
            window[panel]["frame"],
            width=WIDTH["path"],
            height=HEIGHT["path"],
            wraplength=10 * WIDTH["path"] - MARGIN,
            relief="ridge",
            borderwidth=1,
        )
        window[panel]["file label"].grid(
        ...
        window[panel]["folder button"] = gui.Button(
            ...
            height=HEIGHT["path"],
            text="Folder",
            command=partial(set_folder, panel),
        )
        window[panel]["folder button"].grid(column=0, row=2, padx=MARGIN, pady=MARGIN)
        # Folder label
    ...
    window["progress"] = ttk.Progressbar(
        ...
        orient="horizontal",
        mode="determinate",
        length=10 * (2 * width + WIDTH["button"]),
    )
...
def set_folder(panel: str, default: str = None) -> str:
    ...
    folder = filedialog.askdirectory(
        parent=window["root"],
        initialdir=default,
        title="Select " + PANELS[panel] + " folder",
    )
    if folder:
